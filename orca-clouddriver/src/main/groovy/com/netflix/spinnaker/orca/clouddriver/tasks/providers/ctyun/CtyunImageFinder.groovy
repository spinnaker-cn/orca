package com.netflix.spinnaker.orca.clouddriver.tasks.providers.ctyun
...
@Component
class CtyunImageFinder implements ImageFinder {

  final String cloudProvider = "ctyun"
  ...
  Collection<ImageDetails> byName(Stage stage, String packageName, String imageName) {
    ...
    List<CtyunCloudImage> allMatchedImages =
      oortService.findImage(getCloudProvider(), packageName, null, null, ['imageName':imageName])
        .stream()
        .map({ objectMapper.convertValue(it, CtyunCloudImage.class)} )
        .collect(Collectors.toList())

    if (allMatchedImages.size() > 0) {
      CtyunCloudImage latestImage = allMatchedImages[0]
      return Collections.singletonList(latestImage.toCtyunCloudImageDetails())
    } else {
      return null
    }
  }

  private static class CtyunCloudImage {
    @JsonProperty
    String imageName
    ...
    ImageDetails toCtyunCloudImageDetails() {
      JenkinsDetails jenkinsDetails = new JenkinsDetails("", "", "")
      String imageId = attributes.get("imageId").toString()
      return new CtyunImageDetails(imageName, imageId, jenkinsDetails)
    }

  }

  private static class CtyunImageDetails extends HashMap<String, Object> implements ImageDetails{

    CtyunImageDetails (String imageName, String imageId,  JenkinsDetails jenkinsDetails) {
